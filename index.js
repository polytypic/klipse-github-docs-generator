...
const targets = eval(fs.readFileSync(configPath).toString())().map(target =>
  Object.assign(
    {},
    {
...
function process({
  ...
  const body = fs
    ...
    .pipe(s =>
      targets.reduce(
        (s, t) =>
          s.replace(new RegExp(`\\b${esc(t.source)}\\b`, 'g'), t.target),
        s
      )
    )
    ...
    .replace(/\[([^\]]*)\]\(\.\/([^)]*)\)/g, `[$1](${blob}/$2)`)
    .replace(
      new RegExp(esc(`[▶](${site}#`) + '([a-zA-Z0-9-]*)' + esc(')'), 'g'),
      `[■](${blob}/${source}#$1)`
    )
  ...
  for (;;) {
    ...
    if (!m) break
    if (ids.has(m[1])) console.warn(`Duplicate id '${m[1]}' in '${source}'`)
    ids.set(m[1], 1)
  }
  ...
  for (;;) {
    ...
    if (!ids.has(m[1]))
      console.warn(
        `Target of internal link '${m[1]}' does not exist in '${source}'`
      )
  }
...
<html>
  ...
  if (!fs.existsSync(docsFwDir)) fs.mkdirSync(docsFwDir)
